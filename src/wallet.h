...
struct AgeCompare
{
    bool operator ()(const pair<const CWalletTx*, unsigned int> &a, const pair<const CWalletTx*, unsigned int> &b)
    {
printf("a=%d, b=%d\n", a.second, b.second);
...
{
private:
    bool SelectCoins(const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*, unsigned int>, AgeCompare >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl = NULL, AvailableCoinsType coin_type = ALL_COINS, bool useIX = true) const;
...
public:
    ...
    bool SelectCoinsMinConf(const CAmount& nTargetValue, int nConfMine, int nConfTheirs, std::vector<COutput> vCoins, std::set<std::pair<const CWalletTx*, unsigned int>, AgeCompare >& setCoinsRet, CAmount& nValueRet) const;
